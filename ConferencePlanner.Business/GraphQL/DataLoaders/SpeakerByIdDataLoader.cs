...
namespace ConferencePlanner.Business.GraphQL.DataLoaders
{
    public class SpeakerByIdDataLoader : BatchDataLoader<int, Speaker>
    {
        ...
        protected override async Task<IReadOnlyDictionary<int, Speaker>> LoadBatchAsync(IReadOnlyList<int> keys,
            CancellationToken cancellationToken)
        {
            await using ApplicationDbContext dbContext = _dbContextFactory.CreateDbContext();
            return await dbContext.Speakers.Where(s => keys.Contains(s.Id))
